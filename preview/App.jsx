...
import Renderer from "./Renderer";        // Renderer 컴포넌트
...
export default function App() {
  ...
  const [finalIr, setFinalIr] = useState(null);
  ...
  // Compute Final IR
  // -------------------------
  function computeFinalIr(irData, sketchData) {
    if (irData && sketchData) return mergeSketchDomIR(irData, sketchData);
    if (irData) return irData;
    return null;
  }

  // -------------------------
  // Build IR & Merge with Sketch IR
  // -------------------------
  async function buildIR(base, sketchIdParam) {
    ...
    try {
      ...
      if (paths.sketchFile && sketchId) {
        try {
          const sketchResp = await fetch(
            `${API_BASE}/api/sketch-dom-ir?sketchFile=${encodeURIComponent(paths.sketchFile)}&sketchId=${encodeURIComponent(sketchId)}`
          );
          if (sketchResp.ok) {
            const sketchJson = await sketchResp.json();
            sketchData = sketchJson.ir;
            setSketchIr(sketchJson);
          } else {
            setSketchIr(null);
          }
        } catch {
      ...
      const merged = computeFinalIr(irData, sketchData);
      setFinalIr(merged);

      setConvertResult({
        screenId: merged?.screenId || irData.screenId,
        sources: {
          lvglScreens: screens.lvglScreens,
          reactScreens: screens.reactScreens,
          sketchParsed: sketchData ? 1 : 0,
          screenshots: [],
        },
      });

      log("IR 생성 및 Final IR 병합 완료");
    } catch (e) {
  ...
  async function handleShowSketchIr(sketchIdParam) {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        setSketchIr(data);
        log("Sketch IR 조회 성공");
        setFinalIr(computeFinalIr(ir, data.ir));
      } else {
        setSketchIr(null);
        setFinalIr(computeFinalIr(ir, null));
  ...
  const handleShareCode = useCallback(async () => {
    if (!finalIr && !ir) return alert("스크린을 먼저 선택하세요.");

    const zip = new JSZip();
    const code = irToJsx(finalIr || ir, selected || "ScreenMain");
  ...
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "sans-serif" }}>
          ...
          {screens.lvglScreens.length === 0 && screens.reactScreens.length === 0 ? (
            ...
            <>
              {screens.lvglScreens.map(fullPath => {
                const fileName = fullPath.replace(/\\/g, '/').split('/').pop().replace(/\.c$/, '');
                ...
                return (
                  <div key={fileName} style={{
                    ...
                    >
                      {fileName}{sketchId && <span style={{ color: '#38bdf8', fontSize: 12, marginLeft: 6 }}>[Sketch]</span>}
                    </span>
                    {paths.sketchFile && (
                      <input
                        type="text"
                        ...
                        onChange={e => {
                          ...
                          setScreens(s => {
                            ...
                            if (selected === fileName) setTimeout(() => buildIR(fileName, newId), 0);
                            return next;
                          });
                        }}
              ...
              {screens.reactScreens.map(fullPath => {
                const fileName = fullPath.replace(/\\/g, '/').split('/').pop().replace(/\.jsx$/, '');
                return (
                  <div key={fileName} style={{
                    display: 'flex', alignItems: 'center',
                    padding: '7px 10px',
                    ...
                    <span style={{ cursor: 'pointer', flex: 1 }} onClick={() => { setSelected(fileName); buildIR(fileName); }}>
                      {fileName}<span style={{ color: '#a3a3a3', fontSize: 12, marginLeft: 6 }}>[React]</span>
                    </span>
                  </div>
                );
              })}
      ...
      {/* Resizers */}
      <div style={{ width: 7, cursor: 'col-resize', background: '#e0e7ef', zIndex: 10 }}
        onMouseDown={e => { dragging.current = { type: 'left', startX: e.clientX, startWidth: leftWidth }; }} />
      <div style={{
        flex: 1,
        minWidth: 120,
        display: "flex",
        ...
        <div style={{
          ...
          {(finalIr || safeIr) ? <Renderer ir={finalIr || safeIr} /> : <div style={{ color: "red" }}>스크린을 선택하세요.</div>}
        </div>
      </div>
      <div style={{ width: 7, cursor: 'col-resize', background: '#e0e7ef', zIndex: 10 }}
        onMouseDown={e => { dragging.current = { type: 'right', startX: e.clientX, startWidth: rightWidth }; }} />
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', borderBottom: '1.5px solid #e5e7eb', background: '#f9fafb', height: 44 }}>
          {["ir", "sketchIr", "finalIr", "jsx"].map(tab => (
            <div key={tab}
              onClick={() => tab === "sketchIr" ? handleShowSketchIr(selected && screens.sketchIds[selected]) : setPreviewTab(tab)}
              style={{
                flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
                fontWeight: 500, fontSize: 16,
                color: previewTab === tab ? '#2563eb' : '#222',
                borderBottom: previewTab === tab ? '2.5px solid #2563eb' : '2.5px solid transparent',
                background: 'none', transition: 'color 0.15s, border-bottom 0.15s', height: '100%',
              }}
              onMouseOver={e => { if (previewTab !== tab) e.currentTarget.style.color = '#2563eb'; }}
              onMouseOut={e => { if (previewTab !== tab) e.currentTarget.style.color = '#222'; }}
            >
              {tab === "ir" ? "IR Data" : tab === "sketchIr" ? "Sketch IR" : tab === "finalIr" ? "Final IR" : "React Code"}
            </div>
          ))}
        </div>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: 20 }}>
          {previewTab === "ir" && <pre>{safeIr ? JSON.stringify(safeIr, null, 2) : "[IR 없음]"}</pre>}
          {previewTab === "sketchIr" && <pre>{sketchIr ? JSON.stringify(sketchIr, null, 2) : "[Sketch IR 없음]"}</pre>}
          {previewTab === "finalIr" && <pre>{finalIr ? JSON.stringify(finalIr, null, 2) : "[Final IR 없음]"}</pre>}
          {previewTab === "jsx" && <pre>{finalIr ? irToJsx(finalIr, selected || "ScreenMain") : "[React 코드 없음]"}</pre>}
        </div>
      </div>
    </div>
  );
